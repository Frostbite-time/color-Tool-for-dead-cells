...
//读取色带文件和图片文件   (用于单个图片的上色)   本段代码已弃用。。  原因未知（我已经很久没有看这个项目代码了）
void readfiles_only() {
	...
	{	
		...
		if ((got_block == "3"))
		{
			...
			for (int i_1 = 0; i_1 < k_1 + 1; i_1++) 
			{
				...
				cout << "正在处理色带，耗时较久，如果过久无反应，尝试按下回车" << endl;
		...
		if (got_block == "4") 
		{
			...
			for (int k = 0; pic_name[k] != "there is nothing"; k++) {
				for (int l = 0; normal_name[l] != "there is nothing"; l++) {
					...
					string pic_mode = pic_name[k].substr(0, pos_of_number+2);  //截取名称中从位置0到第一个数字位置减去1的字符串，这就是皮肤的模式  如 beheaded1.png 在截取之后为 beheaded ；皮肤的模式将会在后续与法线图的模式比较，如果相同的话就会执行阴影添加。 批量上色之所以能识别不同的皮肤与色带对应关系就是这个原理  
					string normal_mode = normal_name[l].substr(0, normal_name[l].find('_'));    //获取法线图的模式  找到法线图名称中的第一个 _ 字符，然后后退2个位置进行截取  如 beheaded1_n.png 截取后为 beheaded
					...
					//这一段检查是否为R5类    由于获取图片模式的方式是用数字截取，如果文件名有多个数字就可能出错，所以遇到名称类似 beheadedAssassinR51.png 的文件截取后就会变成 beheadedAssassinR 因此我会加上这一段检测； 色带与法线的截取方式是检查 _ 字符，因此不需要这个检查
					if ((pic_name[k].find_last_of("R") >= 0) && (pic_name[k].find_last_of("R") < (pic_name[k].length()))) {
						if (pic_name[k].at(pic_name[k].find_last_of("R")+2) >= '0' && pic_name[k].at(pic_name[k].find_last_of("R")+2) <= '9') {
							pic_mode = pic_name[k].substr(0, getpos_string(pic_name[k])+2);
						}
						//这一段代码首先是检查了名称中是否有字符 R 并且是否位于字符串的长度范围之内。如果都满足，则查看字符 R 后的第2个位置是否是数字，如果是。那么截取到第一个数字位置
						//如 beheadedAssassinR51.png  名称当中有字符 R ，并且字符 R 后第二个位置是字符 1 ，是数字。 那么截取到这个字符串中第一个有数字的位置，而不是减去一的位置  即beheadedAssassinR5
