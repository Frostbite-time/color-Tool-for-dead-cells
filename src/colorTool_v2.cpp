...
const int COLOR_NUM = 65000;     //去色模式下最多可以处理的颜色总数
...
{	
	...
	{	
		...
		if ((got_block == "3"))
		{
			...
			{
				color_pot colors[COLOR_NUM];//一个巨大的结构数组，用于储存临时色带
				...
				for (int j_1 = 0; pic_mode_name[i_1][j_1]!= "there is nothing"; j_1++)
				{
					...
					//对图片进行分割处理，极大提升去色速度
					Mat* pic_spate = new cv::Mat[1000];//准备一个Mat数组，用于储存分割后的图片
					int pic_spate_num_x = 0;//图片的x轴分割点数量
					int pic_spate_num_y = 0;//图片的y轴分割点数量
					//先行计算分割点总数  以512x512为一个分割点
					pic_spate_num_x = src.cols/512;
					pic_spate_num_y = src.rows/512;
					int pic_spate_num = 0;//分割后的图片总数，经过后续的计算后会成为真实总数，当成数组下标需要减少1

					//开始分割图片
					for (int i = 0;i<=pic_spate_num_y;i++)
					{
						for(int j = 0; j <= pic_spate_num_x;j++)
						{
							if(((512*i)!=src.rows)&((512*j)!=src.cols)){  //防止出现裁剪点是图片边缘的情况
								if((512*(i+1))>src.rows){  //这边是四种不同的分割情况，其实就是考虑到可能会有一个地方剩下的不到512
									if ((512*(j+1))>src.cols)
									{
										pic_spate[pic_spate_num] = src(Range(i*512,src.rows),Range(j*512,src.cols));
										pic_spate_num++;
									}
									else
									{
										pic_spate[pic_spate_num] = src(Range(i*512,src.rows),Range(j*512,(j+1)*512));
										pic_spate_num++;
									}
								...
								else
								{
									if ((512*(j+1))>src.cols)
									{
										pic_spate[pic_spate_num] = src(Range(i*512,(i+1)*512),Range(j*512,src.cols));
										pic_spate_num++;
									}
									else
									{
										pic_spate[pic_spate_num] = src(Range(i*512,(i+1)*512),Range(j*512,(j+1)*512));
										pic_spate_num++;
									}
								}
							}	
					...
					for(int p = 0; p<pic_spate_num; p++)   //开始遍历
					{
						for (int i = 0; i < pic_spate[p].rows; i++)   //遍历
						{	
							for (int j = 0; j < pic_spate[p].cols; j++) {    //遍历
								if (pic_spate[p].at<Vec4b>(i, j)[3] != 0) {  //忽略透明部分以增加速度
									//初始化，将第一个颜色读入
									if (pot_ttypes == 0) {   //如果当前颜色种类为0，先储存第一个颜色
										colors[pot_ttypes].red_d = pic_spate[p].at<Vec4b>(i, j)[2];
										colors[pot_ttypes].green_d = pic_spate[p].at<Vec4b>(i, j)[1];
										colors[pot_ttypes].blue_d = pic_spate[p].at<Vec4b>(i, j)[0];
										colors[pot_ttypes].alp = pic_spate[p].at<Vec4b>(i, j)[3];
										pot_ttypes++;
										cout<<"正在处理第"<<pot_ttypes<<"个颜色，如果过久无反应，尝试按下回车"<<endl;
									...
									else
									{
										...
										for (int i = 0; i < pic_spate[p].rows; i++) {    
											for (int j = 0; j < pic_spate[p].cols; j++) {
												if (pic_spate[p].at<Vec4b>(i, j)[3] != 0) {  //忽略透明像素
													if ((colors[pot_ttypes-1].red_d == pic_spate[p].at<Vec4b>(i, j)[2]) && (colors[pot_ttypes-1].green_d == pic_spate[p].at<Vec4b>(i, j)[1]) && (colors[pot_ttypes-1].blue_d == pic_spate[p].at<Vec4b>(i, j)[0])) {
														not_find_same_color = false;     //首先检测上一个存储的颜色是否和这个颜色相同。如果是，那么直接设置为假，并且将图片的这部分设为透明以加快下一次循环到此处的速度
														pic_spate[p].at<Vec4b>(i, j)[3] = 0;      //事实证明这个设置为透明非常有用，将速度从5分钟提升到了50秒

													}
													else {     //如果上一个存储的颜色不同的话，再检测已经存储的所有颜色中是否有相同的。以免遗漏
														for (int times_i = 0; times_i < pot_ttypes; times_i++) {
															if ((colors[times_i].red_d == pic_spate[p].at<Vec4b>(i, j)[2]) && (colors[times_i].green_d == pic_spate[p].at<Vec4b>(i, j)[1]) && (colors[times_i].blue_d == pic_spate[p].at<Vec4b>(i, j)[0])) {
																not_find_same_color = false;
																pic_spate[p].at<Vec4b>(i, j)[3] = 0;//将相同的颜色设置为透明，增加下一次循环的速度
										...
										if (not_find_same_color)   //如果真的找到了不同的颜色，将这个不同的颜色存储到颜色列表，并且增加颜色的种类
										{
											...
											pot_ttypes++;
											colors[pot_ttypes - 1].red_d = pic_spate[p].at<Vec4b>(i, j)[2];
											colors[pot_ttypes - 1].green_d = pic_spate[p].at<Vec4b>(i, j)[1];
											colors[pot_ttypes - 1].blue_d = pic_spate[p].at<Vec4b>(i, j)[0];
											colors[pot_ttypes - 1].alp = pic_spate[p].at<Vec4b>(i, j)[3];
											cout<<"正在处理第"<<pot_ttypes<<"个颜色，如果过久无反应，尝试按下回车"<<endl;
		...
		if (got_block == "4") 
		{
			...
			for (int k = 0; pic_name[k] != "there is nothing"; k++) {
				for (int l = 0; normal_name[l] != "there is nothing"; l++) {
					...
					string pic_mode = pic_name[k].substr(0, pic_name[k].find_last_of("."));  //截取名称中从位置0到字符"."字符串，这就是皮肤的模式  如 beheaded1.png 在截取之后为 beheaded1 ；皮肤的模式将会在后续与法线图的模式比较，如果相同的话就会执行阴影添加。 批量上色之所以能识别不同的皮肤与色带对应关系就是这个原理  
					string normal_mode = normal_name[l].substr(0, normal_name[l].find_last_of('.')-2);    //获取法线图的模式  找到法线图名称中的第一个 . 字符，然后后退2个位置进行截取  如 beheaded1_n.png 截取后为 beheaded1
